...
def calculate_therapy(weight, dose, separator, flask_type, plasma_priming, media_freq):
    ...
    passages.append({'passage_num': 0, 'flasks': initial_flasks, 'input': initial_flasks * FLASK_TYPES[flask_type]['seeding'], 'output': initial_flasks * FLASK_TYPES[flask_type]['confluent'], 'days': 0, 'media_changes': 0})

    for passage_num in range(1, MAX_PASSAGES + 1):
        if passages[-1]['output'] >= target_cells:
            break
        
        current_flasks = math.ceil((passages[-1]['output'] / FLASK_TYPES[flask_type]['seeding']) * SAFETY_FACTOR)
        days = PASSAGE1_DAYS if passage_num == 1 else PASSAGE_DAYS
        media_changes = (days // media_freq) + 1  # +1 for initial media
        
        passages.append({'passage_num': passage_num, 'flasks': current_flasks, 'input': current_flasks * FLASK_TYPES[flask_type]['seeding'], 'output': current_flasks * FLASK_TYPES[flask_type]['confluent'], 'days': days, 'media_changes': media_changes})
    ...
    return {'pbsc_ml': pbsc_ml, 'passages': passages, 'total_days': total_days, 'total_media': total_media, 'plasma_vol': plasma_vol, 'target_cells': target_cells, 'final_yield': passages[-1]['output']}
